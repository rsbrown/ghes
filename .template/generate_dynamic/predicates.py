...
def have_safari_icons(build):
	return "icons" in build.config and \
		"32" in build.config["icons"] and \
		"48" in build.config["icons"] and \
		"64" in build.config["icons"]

@predicate
def have_android_icons(build):
	return "icons" in build.config and \
		"36" in build.config["icons"] and \
		"48" in build.config["icons"] and \
		"72" in build.config["icons"]
...
def have_ios_icons(build):
	return "icons" in build.config and \
		"57" in build.config["icons"] and \
		"72" in build.config["icons"] and \
		"114" in build.config["icons"]
